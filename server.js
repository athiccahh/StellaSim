...
// this holds the local telescope bridge state
const telescopeBridgeState = {
  mode: "real",
  connected: false,
  bridgeStatus: "idle",
  currentAlt: 0,
...
// this updates the bridge timestamp
function touchTelescopeBridge() {
  telescopeBridgeState.updatedAt = new Date().toISOString();
}
...
// this returns the telescope bridge status
app.get("/api/telescope/status", (req, res) => {
  touchTelescopeBridge();
  res.json({
    ok: true,
    status: telescopeBridgeState.bridgeStatus,
    telescope: telescopeBridgeState
  });
});

// this marks the local bridge as connected
app.post("/api/telescope/connect", (req, res) => {
  telescopeBridgeState.connected = true;
  telescopeBridgeState.bridgeStatus = "connected";
  telescopeBridgeState.slewing = false;
  telescopeBridgeState.lastCommand = {
    type: "connect",
...
// this disconnects the local bridge state
app.post("/api/telescope/disconnect", (req, res) => {
  telescopeBridgeState.connected = false;
  telescopeBridgeState.bridgeStatus = "idle";
  telescopeBridgeState.tracking = false;
  telescopeBridgeState.slewing = false;
  telescopeBridgeState.selectedObjectName = null;
  telescopeBridgeState.lastCommand = {
    type: "disconnect",
    at: new Date().toISOString()
  };
...
// this accepts a slew target from stellasim
app.post("/api/telescope/slew", (req, res) => {
  const alt = clampNumber(numberOrNull(req.body?.alt) ?? 0, -89, 89);
  const az = clampNumber(numberOrNull(req.body?.az) ?? 0, 0, 360);
  const label = cleanText(req.body?.label, "Custom Target");

  telescopeBridgeState.connected = true;
  telescopeBridgeState.bridgeStatus = "slew_sent";
  telescopeBridgeState.slewing = true;
  telescopeBridgeState.targetAlt = alt;
  telescopeBridgeState.targetAz = az;
  telescopeBridgeState.selectedObjectName = label;
  telescopeBridgeState.lastCommand = {
    type: "slew",
    label,
    alt,
  ...
  res.json({
    ok: true,
    status: telescopeBridgeState.bridgeStatus,
    targetAlt: telescopeBridgeState.targetAlt,
    targetAz: telescopeBridgeState.targetAz,
    telescope: telescopeBridgeState
  });
});

// this lets the frontend update the current mount pointing later
app.post("/api/telescope/telemetry", (req, res) => {
  const alt = numberOrNull(req.body?.currentAlt);
  const az = numberOrNull(req.body?.currentAz);
  const slewing = typeof req.body?.slewing === "boolean" ? req.body.slewing : telescopeBridgeState.slewing;
  const tracking = typeof req.body?.tracking === "boolean" ? req.body.tracking : telescopeBridgeState.tracking;

  if (alt !== null) telescopeBridgeState.currentAlt = clampNumber(alt, -89, 89);
  if (az !== null) telescopeBridgeState.currentAz = clampNumber(az, 0, 360);

  telescopeBridgeState.slewing = slewing;
  telescopeBridgeState.tracking = tracking;
  telescopeBridgeState.connected = true;
  telescopeBridgeState.bridgeStatus = "telemetry_updated";
  telescopeBridgeState.lastCommand = {
    type: "telemetry",
    at: new Date().toISOString()
  };
...
// this toggles tracking state for the local bridge
app.post("/api/telescope/tracking", (req, res) => {
  const enabled = typeof req.body?.enabled === "boolean"
    ? req.body.enabled
    : !telescopeBridgeState.tracking;

  telescopeBridgeState.connected = true;
  telescopeBridgeState.tracking = enabled;
  telescopeBridgeState.bridgeStatus = enabled ? "tracking_on" : "tracking_off";
  telescopeBridgeState.lastCommand = {
    type: "tracking",
    enabled,
    at: new Date().toISOString()
